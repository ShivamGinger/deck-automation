...
const RenderParametersUnderQuotient = ({
  quotientName,
  parametersUnderQuotients
}: {
  quotientName: string,
  parametersUnderQuotients: ParametersQuotientFactors[]
}) => {
  ...
  const totalItems = parametersUnderQuotients?.length || 0;
  const totalPages = Math.ceil(totalItems / ITEMS_PER_PAGE);

  const startIndex = currentPage * ITEMS_PER_PAGE;
  const endIndex = Math.min(startIndex + ITEMS_PER_PAGE, totalItems);

  const currentData = parametersUnderQuotients?.slice(startIndex, endIndex);
      ...
      <div className='flex flex-col'>
        <div className="rounded-lg shadow overflow-x-auto bg-white">
          ...
          <div className='p-4 flex flex-row justify-between'>
            <h2 className='font-bold uppercase text-2xl text-[#542C06]'>Parameter details for {quotientName}</h2>
            {/* <div className='p-2'>
              search ...
            </div> */}
          </div>
          <table className="w-full border-separate border-spacing-4 px-6 pt-2">
            <thead className="">
              <tr className='gap-x-4'>
                ...
                <th className="table-headings">Parameter Weightage</th>
              </tr>
            </thead>
            <tbody className="">
...
export default RenderParametersUnderQuotient
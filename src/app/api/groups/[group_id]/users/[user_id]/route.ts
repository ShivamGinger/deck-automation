...
export async function DELETE(request: Request, { params }: { params: { group_id: number, user_id: number } }) {
    try {
        const grpSlug = params.group_id;
        const userSlug = params.user_id;
        ...
        await db.delete(userGroups).where(and(eq(userGroups.userId, userSlug), eq(userGroups.groupId, grpSlug)));
